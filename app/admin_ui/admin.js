...
const api = async (url, options = {}) => {
  ...
  if (response.status === 401 || response.redirected) { location.href = '/admin/login'; throw new Error('Session expired'); }
...
async function load() {
  const [documents, readiness] = await Promise.all([api('/admin/data/documents'), api('/admin/data/readiness')]);
  ...
  for (const doc of documents) {
    ...
    const reindex = document.createElement('button'); reindex.textContent = 'Reindex'; reindex.onclick = () => act(`/admin/data/documents/${doc.id}/reindex`, 'POST');
    const remove = document.createElement('button'); remove.textContent = 'Delete'; remove.className = 'danger'; remove.onclick = () => { if (confirm(`Delete ${doc.filename}? This cannot be undone.`)) act(`/admin/data/documents/${doc.id}`, 'DELETE'); };
...
document.querySelector('#upload').addEventListener('submit', async event => { event.preventDefault(); const button = event.currentTarget.querySelector('button'); button.disabled = true; try { await api('/admin/data/documents', {method:'POST', body:new FormData(event.currentTarget)}); event.currentTarget.reset(); notice.textContent='Document uploaded'; await load(); } catch(error) { notice.textContent=error.message; } finally { button.disabled=false; } });
document.querySelector('#reindex-all').onclick = () => act('/admin/data/documents/reindex-incompatible', 'POST');
